...
## 16. コンテンツ作成ガイドレイン（AI指示用）

ブログ記事やドキュメントを生成する際は、以下のルールを遵守すること。

* **太字禁止**：日本語の文章内では `**太字**` を使用しない。（記号が重なり文字化けやレンダリング崩れの原因となるため）。強調したい場合は「」か、文脈で強調する。
* **文体**：ZINE文化に寄せた、少し砕けた柔らかい敬語（「〜です・ます」調だが堅苦しくない）を使用する。
```
